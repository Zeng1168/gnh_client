<template>
    ...
    <el-main>
        <el-row :gutter="8" style="margin:0 auto;margin-top:2%; width:80%;">
            ...
            <el-col :xs="24" :sm="18" :md="12" :lg="12" :xl="7">
                <el-card class="box-card" style="min-height:60vh; background-color:rgba(253,253,253,0.8);">
                    ...
                    <el-form style="min-height:50vh;" ref="formreg" label-width="80px">
                        ...
                        <el-form-item >
                            <el-button type="primary" style="width:60%;" @click="register">注 册</el-button>
...
export default {
    ...
    methods: {
        register(){
            if(!this.studentId | !this.name | !this.bindPhone |  (this.password2==='') | (this.password2==='')){
                this.warnMsg("请填写完整");
            }
            ...
            // }
                 ...
                 console.log("注册");
                 this.$axios.post('/userAccount/userSign',this.qs.stringify({
       ...
       //获取验证码
        VerificationCodef(){
            if(!(/^1(3|4|5|6|7|8|9)\d{9}$/.test(this.bindPhone))){
               ...
               //发送请求验证码
               this.$axios.get('/userAccount/getSignSms?phone='+this.bindPhone)
               .then(res =>{
                   if(res.data.status===0){
                    //    this.tureVerificationCode = res.data.msg;
                       console.log(res);
                    //    console.log(this.tureVerificationCode);
                       this.succMsg(res.data.msg);
                        this.countDown(60); 
                   }
                   else {
